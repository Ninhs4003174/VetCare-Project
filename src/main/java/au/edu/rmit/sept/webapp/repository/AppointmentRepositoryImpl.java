...
public List<Appointment> findAll() {
    try {
        ...
        while (rs.next()) {
            ...
            Appointment appt = new Appointment(rs.getLong(1), rs.getString(2), rs.getLong(3), rs.getString(4), rs.getString(5), rs.getString(6), null);
    ...
    try {
        ...
        public List<Appointment> findByUser(User user) {
            try {
                ...
                List<Appointment> appointments = new ArrayList<>();
                while (rs.next()) {
                    Appointment appt = new Appointment(rs.getLong(1), rs.getString(2), rs.getLong(3), rs.getString(4), rs.getString(5), rs.getString(6), user);
                    appointments.add(appt);
                }
                connection.close();
                return appointments;
    ...
    try {
        ...
        if (appointment.getId() == null) {
            PreparedStatement insertStm = connection.prepareStatement(
                "INSERT INTO appointments (pet_name, vet_id, date, time, status, user_id) VALUES (?, ?, ?, ?, ?, ?)"
            );
            insertStm.setString(1, appointment.getPetName());
            insertStm.setLong(2, appointment.getVetId());
            ...
            PreparedStatement updateStm = connection.prepareStatement(
                "UPDATE appointments SET pet_name = ?, vet_id = ?, date = ?, time = ?, status = ?, user_id = ? WHERE id = ?"
            );
            updateStm.setString(1, appointment.getPetName());
            updateStm.setLong(2, appointment.getVetId());
...
public Appointment findById(Long id) {
    try {
        ...
        ResultSet rs = stm.executeQuery();
        if (rs.next()) {
            Appointment appointment = new Appointment(rs.getLong(1), rs.getString(2), rs.getLong(3), rs.getString(4), rs.getString(5), rs.getString(6), null);
            connection.close();
            return appointment;
        } else {
            connection.close();
